"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var Animate = /** @class */ (function () {
    function Animate() {
        this.running = {};
        this.counter = 1;
        this.time = Date.now || function () {
            return +new Date();
        };
    }
    ;
    Animate.prototype.animate = function (callback) {
        var requestFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame;
        requestFrame(callback);
    };
    Animate.prototype.stop = function (id) {
        var stop = this.running[id] !== null;
        stop && (this.running[id] = null);
        return stop;
    };
    Animate.prototype.isRunning = function (id) {
        return this.running[id] !== null;
    };
    Animate.prototype.start = function (stepCb, verifyCb, completedCb, duration, easingMethod) {
        var _this = this;
        var start = this.time(); // 获取开始时的时间
        var lastFrame = start;
        var percent = 0;
        var dropCounter = 0;
        var id = this.counter++; // 动画Id
        // 压缩运行数据库自动每几个新的动画
        if (id % 20 === 0) {
            var newRunning = {};
            for (var usedId in this.running) {
                newRunning[usedId] = true;
            }
            this.running = newRunning;
        }
        // 这是每隔几毫秒调用一次的内部步骤方法
        var step = function (virtual) {
            var render = virtual !== true; // 初始化虚拟值
            var now = _this.time(); // 获取当前时间
            // 验证在下一个动画步骤之前执行
            if (!_this.running[id] || (verifyCb && !verifyCb(id))) {
                _this.running[id] = null;
                var renderedSecond = 60 - dropCounter / ((now - start) / 1000);
                if (!!completedCb) {
                    completedCb(renderedSecond, id, false);
                }
                return;
            }
            // 要应用当前呈现，让我们更新内存中省略的步骤。 这对于及时更新内部状态变量非常重要。
            if (render) {
                var droppedFrames = Math.round((now - lastFrame) / (1000 / 60)) - 1;
                for (var j = 0; j < Math.min(droppedFrames, 4); j++) {
                    step(true);
                    dropCounter++;
                }
            }
            // 计算百分比值
            if (duration) {
                percent = (now - start) / duration;
                if (percent > 1) {
                    percent = 1;
                }
            }
            // 执行步骤回调, then...
            var value = easingMethod ? easingMethod(percent) : percent;
            if ((stepCb(value, now, render) === false || percent === 1) && render) {
                _this.running[id] = null;
                var renderedSecond = 60 - dropCounter / ((now - start) / 1000);
                if (!!completedCb) {
                    completedCb(renderedSecond, id, percent === 1 || duration == null);
                }
            }
            else if (render) {
                lastFrame = now;
                _this.animate(step);
            }
        };
        // Mark as running
        this.running[id] = true;
        // 初始化的第一步
        this.animate(step);
        // 返回唯一的动画ID
        return id;
    };
    return Animate;
}());
exports.default = new Animate();
//# sourceMappingURL=Animate.js.map